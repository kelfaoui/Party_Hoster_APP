...
const Commentaire_js_1 = __importDefault(require("../models/Commentaire.js"));
class CommentaireController {
    ...
    static async getAll(req, res) {
        try {
            ...
            const commentaires = await Commentaire_js_1.default.findAll(Number(limit), Number(offset));
     ...
     *               $ref: '#/components/schemas/Error'
     */
    static async getOwnerCommentaires(req, res) {
        try {
            const { limit = 100, offset = 0 } = req.query;
            ...
            const commentaires = await Commentaire_js_1.default.findByOwnerId(req.user.utilisateur_id, Number(limit), Number(offset));
            res.json(commentaires);
        }
        catch (error) {
            res.status(500).json({ message: error.message });
    ...
    static async create(req, res) {
        try {
            ...
            const commentaireId = await Commentaire_js_1.default.create(commentaireData);
            res.status(201).json({
    ...
    static async getBySalle(req, res) {
        try {
            ...
            const commentaires = await Commentaire_js_1.default.findBySalleId(salleId);
            res.json(commentaires);
        }
        catch (error) {
            res.status(500).json({ message: error.message });
    ...
    static async update(req, res) {
        try {
            ...
            const commentaireObj = await Commentaire_js_1.default.findById(commentaireId);
            if (!commentaireObj) {
            ...
            await Commentaire_js_1.default.update(commentaireId, commentaire);
    ...
    static async delete(req, res) {
        try {
            const commentaireId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const commentaire = await Commentaire_js_1.default.findById(commentaireId);
            if (!commentaire) {
                res.status(404).json({ message: 'Commentaire non trouvé' });
                return;
            }
            ...
            await Commentaire_js_1.default.delete(commentaireId);
...
exports.default = CommentaireController;
//# sourceMappingURL=commentaireController.js.map