...
class NotationController {
    ...
    static async createOrUpdate(req, res) {
        try {
            ...
            const existingNotation = await Notation.findByUtilisateurAndSalle(req.user.utilisateur_id, salle_id);
            let notationId;
            if (existingNotation) {
                await Notation.update(existingNotation.notation_id, note);
    ...
    static async getMaNotation(req, res) {
        try {
            const salleId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
            const notation = await Notation.findByUtilisateurAndSalle(req.user.utilisateur_id, salleId);
            res.json(notation || {});
        }
        catch (error) {
            res.status(500).json({ message: error.message });
    ...
    static async delete(req, res) {
        try {
            ...
            const notation = await Notation.findById(notationId);
            if (!notation) {
