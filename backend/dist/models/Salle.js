...
class Salle {
    static async create(salle) {
        const [result] = await database_js_1.pool.execute(`INSERT INTO salles (nom, utilisateur_id, image, description, capacite, 
    ...
    static async findById(id) {
        const [rows] = await database_js_1.pool.execute('SELECT * FROM salles WHERE salle_id = ?', [id]);
        return rows[0] || null;
    }
    static async findAll(filters = {}, limit = 100, offset = 0) {
        ...
        try {
            const [rows] = await database_js_1.pool.execute(query, params);
            return rows;
        }
        catch (error) {
    ...
    static async update(id, salle) {
        const [result] = await database_js_1.pool.execute(`UPDATE salles 
    ...
    static async delete(id) {
        const [result] = await database_js_1.pool.execute('DELETE FROM salles WHERE salle_id = ?', [id]);
        return result.affectedRows;
    }
    static async getSallesDisponibles(debut, fin) {
        const [rows] = await database_js_1.pool.execute(`SELECT s.* FROM salles s
